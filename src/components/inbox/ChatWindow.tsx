    ...
    <section className="flex-1 bg-[#f8f9fa] flex flex-col relative">
      ...
      <header className="h-16 flex items-center justify-between px-8 bg-white/70 backdrop-blur-md sticky top-0 z-10 border-b border-[#e1e3e4]">
        <div className="flex items-center gap-4">
          {onToggleList && (
            ...
            >
              <span className="material-symbols-outlined">{listMode === 'open' ? 'menu_open' : 'menu'}</span>
            </button>
          )}
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-headline font-bold text-lg leading-tight text-[#003752]">{contact?.name || contactPhone}</h1>
              <div className="flex items-center gap-1.5 mt-0.5">
                <span className="text-[11px] font-bold text-[#727780] uppercase tracking-wider">Online</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="w-10 h-10 flex items-center justify-center rounded-xl bg-[#1d4e6c] text-white hover:opacity-90 transition-all shadow-sm">
          ...
          {onToggleDetails && (
            <button 
              ...
              className={`w-10 h-10 flex items-center justify-center rounded-xl transition-all shadow-sm ${
                !isDetailsCollapsed ? 'bg-[#003752] text-white' : 'bg-[#edeeef] text-[#727780] hover:bg-[#e7e8e9]'
              }`}
            >
              <span className="material-symbols-outlined" style={{ fontVariationSettings: !isDetailsCollapsed ? "'FILL' 1" : "" }}>info</span>
            </button>
          )}
        </div>
      </header>
      ...
      >
        ...
        {loading && messages.length === 0 && (
          <div className="text-center p-8 text-[#727780] text-xs font-medium">Fetching history...</div>
        )}
      </div>
      ...
      <footer className="p-6 bg-white border-t border-[#e1e3e4]">
        <div className="max-w-4xl mx-auto flex flex-col gap-3">
          <div className="flex gap-2 mb-1 overflow-x-auto no-scrollbar">
            {['Quick Hello', 'Pricing Details', 'Meeting Link', 'Next Steps'].map((chip) => (
              <button 
                key={chip}
          ...
          <div className="bg-white rounded-2xl p-2 flex items-end gap-3 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07)] border border-[#e1e3e4] focus-within:border-[#003752]/30 transition-all">
            ...
            <div className="flex items-center gap-2 pb-1 pr-1">
              ...
              >
                <span className="material-symbols-outlined">{isSending ? 'sync' : 'send'}</span>
              </button>
            </div>
          </div>
        </div>
