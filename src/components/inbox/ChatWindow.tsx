...
import { Lock, Unlock, Phone, Search, MoreVertical, Menu, Bell, HelpCircle } from 'lucide-react';
...
const ChatWindow = ({ contactPhone }: ChatWindowProps) => {
  ...
  // Back to Mock Messages for quick testing without DB
  const mockMessages = [
    { id: '1', text: "Hello! Home can I help you today?", sender: 'them', time: '12:45 PM' },
      ...
      <header className="h-14 flex items-center justify-between px-4 bg-[#15191d] border-b border-[#24292e] text-white z-40 shrink-0">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 cursor-pointer hover:text-[#ff9900] transition-colors">
            ...
            <span className="text-sm font-bold tracking-tight">Chat Console (Mock)</span>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-[#1c2128] border border-[#3b4149] flex items-center justify-center text-[10px] font-bold text-[#ff9900]">
              {contactPhone.charAt(0)}
            </div>
            <div className="flex flex-col">
              <span className="text-xs font-bold text-white leading-none">{contactPhone}</span>
              <span className="text-[9px] font-medium text-emerald-500 mt-0.5 flex items-center gap-1">
                <span className="h-1 w-1 rounded-full bg-emerald-500 shadow-sm" /> Connected (Mock)
              </span>
            </div>
          </div>
        </div>
