...
const Sidebar = ({ activeTab = "Chat", onTabChange }: SidebarProps) => {
  const navItems = [
  ...
  return (
    <aside className="fixed left-0 top-0 h-full z-50 bg-[#e7e8e9] text-[#003752] font-headline font-semibold tracking-tight w-20 flex flex-col items-center py-6">
      ...
      <nav className="flex flex-col gap-6 w-full items-center">
        ...
        {/* Bottom Settings */}
        <div
          className="mt-auto text-slate-400 p-3 hover:bg-slate-200 transition-colors rounded-xl cursor-pointer group"
          title="Settings"
        >
          <span className="material-symbols-outlined group-hover:text-[#003752]">
            settings
          </span>
        </div>
